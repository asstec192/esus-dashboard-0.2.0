...
import { type Turn } from "@/hooks/useTurnStore";
 ...
 * @returns
 */
export const getTurnFilterQuery = (dateRange: DateRange, turn: Turn) => {
...
export const getTurnFilterMaisDeUmDia = (dateRange: DateRange, turn: Turn) => {
...
export const getTurnFilterUmdia = (dateRange: DateRange, turn: Turn) => {
  const turnoComViradaDeDia = turn.numericFrom >= turn.numericTo;
  const from = format(dateRange.from, `yyyy-MM-dd ${turn.from}`);
  const to = turnoComViradaDeDia
    ? format(dateRange.to, `yyyy-MM-dd ${turn.to}`)
